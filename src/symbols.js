...
//
// NOTE: most glyphs here are a deliberately simplified stand-in (basic
...
//
// EXCEPTION: the notification-appliances family (below) is traced directly
// from Tim's own reference sheet, at his request — those are common,
// generic notification-appliance symbols used across many CAD/BIM programs
// (not NFPA's own proprietary pictograms), and he wants this app's copies to
// match what he actually drew, not a simplified restyling of it. See the
// comment right above that family for how the tracing was done.
...
const COLORS = {
  ...
  'notification-appliances': '#ef4444', // red -- matches Tim's own reference sheet (NOTIFICATION_DEVICES.pdf), which draws every notification-appliance symbol in red; a distinct shade from heat-detection's red (#dc2626) and abort-switches' dark red (#b91c1c) so the three don't read as the same color in the legend
...
// ---- notification-appliances family, traced from Tim's own reference sheet
// (NOTIFICATION_DEVICES.pdf, retraced Aug 2026). Per Tim: these are common,
// generic fire-alarm notification symbols used across many CAD/BIM
// platforms, not proprietary NFPA artwork, and the visuals need to match
// what he actually drew. So instead of the simplified geometric stand-ins
// this file used elsewhere, the shapes below were measured directly off the
// PDF's own vector paths (pulled with `pdftocairo -svg` and read back
// programmatically) -- exact proportions and exact relative positions for
// the triangle/square/circle/line elements, not eyeballed from a raster.
// Two things were deliberately NOT replayed literally: (1) the reference's
// "solid" triangles are actually a hollow outline filled in with a dense
// freehand pencil scribble (Tim hand-drew these in Bluebeam) -- reproduced
// here as a plain solid fill, which is what that scribble reads as at any
// normal viewing size; (2) text labels use this app's own text() helper
// rather than tracing individual letterforms.
//
// Key shape facts confirmed by measuring the actual path data (not just
// eyeballing the raster render):
//  - The triangle's base sits flush on the square's top edge -- no gap.
//  - Speaker/sounder triangles are SOLID fill; horn triangles are HOLLOW
//    outline only. That distinction is consistent across every icon in the
//    reference and is preserved here.
//  - The square's strobe glyph is a small open circle with 4 SEPARATE
//    diagonal ticks running from just inside each corner in toward the
//    circle, stopping at its edge -- not one continuous X drawn through a
//    circle (that was this file's old, incorrect version of this glyph).
//  - The round strobe-only glyph is that same open-circle-with-4-ticks
//    glyph on its own (no enclosing square), just with longer ticks -- NOT
//    an 8-ray sunburst (that was also this file's old, incorrect version).
//    Wall mount adds a short stand (a vertical leg + a foot) below the
//    circle; ceiling mount has nothing to stand on, so it's bare.

// Circle-with-4-diagonal-ticks strobe glyph, sized either to sit inside a
// square (pass its bounding box) or standalone (pass a synthetic box
// centered on the desired circle). `ringOnly` skips the corner ticks,
// leaving just the ring -- unused today but kept for symmetry with
// standaloneStrobeGlyph below, which needs the ticks to run further out
// than a corner-inset would allow.
function circleTicksGlyph(color, x, y, w, h) {
  const cx = x + w / 2;
  const cy = y + h / 2;
  const r = w * 0.145;
  const inset = w * 0.16;
  ...
  for (const [cornerX, cornerY] of corners) {
    ...
    const ux = dx / len;
    const uy = dy / len;
    const sx = (cornerX + ux * inset).toFixed(1);
    const sy = (cornerY + uy * inset).toFixed(1);
    const ex = (cx - ux * (r + gap)).toFixed(1);
    const ey = (cy - uy * (r + gap)).toFixed(1);
    out += `<line x1="${sx}" y1="${sy}" x2="${ex}" y2="${ey}" stroke="${color}" stroke-width="1.1"/>`;
  }
  out += `<circle cx="${cx.toFixed(1)}" cy="${cy.toFixed(1)}" r="${r.toFixed(1)}" fill="none" stroke="${color}" stroke-width="1.1"/>`;
  return out;
}

// Standalone version of the same glyph (the round strobe-only appliance) --
// same open-circle-with-4-ticks shape, but the ticks run a fixed length
// outward from the circle instead of reaching toward a square's corners.
function standaloneStrobeGlyph(color, cx, cy, r, rayLen) {
  let out = '';
  const rIn = r + 0.3;
  const rOut = rIn + rayLen;
  for (const deg of [45, 135, 225, 315]) {
    ...
    out += `<line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" stroke="${color}" stroke-width="1.2"/>`;
  }
  out += `<circle cx="${cx}" cy="${cy}" r="${r}" fill="none" stroke="${color}" stroke-width="1.2"/>`;
  return out;
}

function downTriangle(color, cx, topY, halfW, h, filled) {
  const points = `${cx - halfW},${topY} ${cx + halfW},${topY} ${cx},${topY + h}`;
  return `<polygon points="${points}" fill="${filled ? color : 'none'}" stroke="${color}" stroke-width="1.2" stroke-linejoin="round"/>`;
}

// Triangle (audible component) sitting flush on a square (housing) -- the
// shared body plan for every notification appliance in the reference except
// the round strobe-only one. `inner` is either 'x' (visible/strobe
// component -> the circle-with-ticks glyph) or a short string (a plain
// letter/tag, e.g. 'S' or 'F', for an audible-only device). `topLeft`/
// `topRight` are the short callouts the reference draws directly beside the
// triangle (e.g. "CD" and "W", or just ".5W"); `bottomLabel1`/`bottomLabel2`
// stack below the square (mount word, then WP) -- drawn below rather than
// beside the square (unlike the reference) purely so the full word fits
// inside this app's fixed small icon frame without clipping.
function triSquareIcon(color, { filled, inner, topLeft, topRight, bottomLabel1, bottomLabel2 }) {
  const sqX = 10,
    sqY = 12,
    sqW = 12,
    sqH = 11;
  const triH = 6.4,
    triHalfW = 4.6;
  const cx = sqX + sqW / 2;
  let out = downTriangle(color, cx, sqY - triH, triHalfW, triH, filled);
  out += `<rect x="${sqX}" y="${sqY}" width="${sqW}" height="${sqH}" fill="none" stroke="${color}" stroke-width="1.3"/>`;
  out += inner === 'x' ? circleTicksGlyph(color, sqX, sqY, sqW, sqH) : txt(cx, sqY + sqH / 2 + 2.6, fitSize(inner, 8), inner, color);
  const triMidY = sqY - triH / 2 + 1.6;
  if (topLeft) out += txt(sqX - 3, triMidY, 5.6, topLeft, color, 600);
  if (topRight) out += txt(sqX + sqW + 3, triMidY, 5.6, topRight, color, 600);
  let y = 27;
  if (bottomLabel1) {
    out += txt(cx, y, 4.7, bottomLabel1, color, 600);
    y += 4.4;
  }
  if (bottomLabel2) out += txt(cx, y, 4.4, bottomLabel2, color, 600);
  return out;
}

// Round strobe-only appliance: an open circle with 4 diagonal ticks (see
// standaloneStrobeGlyph above). The reference draws a real, deliberate
// shape difference between mounts here (not just a different letter) --
// wall-mount gets a short stand (leg + foot) below the circle, ceiling-mount
// doesn't, since a ceiling unit has nothing to stand on.
function strobeSunburstIcon(color, { wall, wp }) {
  const cx = 13,
    cy = 12,
    r = 3.3;
  let out = standaloneStrobeGlyph(color, cx, cy, r, 2.3);
  if (wall) {
    const standBottom = cy + r + 4.2;
    out += `<line x1="${cx}" y1="${(cy + r).toFixed(1)}" x2="${cx}" y2="${standBottom.toFixed(1)}" stroke="${color}" stroke-width="1.3"/>`;
    out += `<line x1="${(cx - 2.4).toFixed(1)}" y1="${standBottom.toFixed(1)}" x2="${(cx + 2.4).toFixed(1)}" y2="${standBottom.toFixed(1)}" stroke="${color}" stroke-width="1.3"/>`;
  }
  out += txt(cx + r + 8, cy - r + 3, 6.2, 'CD', color, 600);
  let y = wall ? cy + r + 11 : cy + r + 6.5;
  out += txt(cx + 1, y, 5.3, wall ? 'W' : 'C', color, 700);
  if (wp) out += txt(cx + 1, y + 5, 4.8, 'WP', color, 600);
  return out;
}
...
export const SYMBOLS = [
  ...
  // ---- Notification Appliances -- rebuilt from Tim's own reference sheet
  // (NOTIFICATION_DEVICES.pdf, traced Aug 2026), replacing the earlier
  // NFPA-170-table-derived set wholesale. See the icon-helper comment above
  // (circleXGlyph/downTriangle/triSquareIcon/strobeSunburstIcon) for the
  // tracing approach.
  //
  // NOTE on the Speaker Strobe wall/ceiling pair: the source PDF's own
  // "DESCRIPTION:" text and the mount word printed on the icon graphic
  // itself disagree with each other (consistently swapped, both for the
  // regular and weatherproof versions) -- confirmed with Tim, and per his
  // call this follows the word actually drawn on the icon, not the
  // description text beside it.
  E('na-speaker-strobe-ceiling', 'Speaker strobe, ceiling mount', 'notification-appliances', 'CD/1W', triSquareIcon(naColor, { filled: true, inner: 'x', topLeft: 'CD', topRight: 'W', bottomLabel1: 'CEILING' })),
  E('na-speaker-strobe-wall', 'Speaker strobe, wall mount', 'notification-appliances', 'CD/1W', triSquareIcon(naColor, { filled: true, inner: 'x', topLeft: 'CD', topRight: 'W', bottomLabel1: 'WALL' })),
  E('na-speaker-wall', 'Speaker only, wall mount', 'notification-appliances', 'S/.5W', triSquareIcon(naColor, { filled: true, inner: 'S', topRight: '.5W', bottomLabel1: 'WALL' })),
  E('na-speaker-ceiling', 'Speaker only, ceiling mount', 'notification-appliances', 'S/.5W', triSquareIcon(naColor, { filled: true, inner: 'S', topRight: '.5W', bottomLabel1: 'CEILING' })),
  E('na-strobe-wall', 'Strobe only, wall mount', 'notification-appliances', 'CD', strobeSunburstIcon(naColor, { wall: true })),
  E('na-strobe-ceiling', 'Strobe only, ceiling mount', 'notification-appliances', 'CD', strobeSunburstIcon(naColor, { wall: false })),
  E('na-horn-only', 'Horn only', 'notification-appliances', 'F/H', triSquareIcon(naColor, { filled: false, inner: 'F', bottomLabel1: 'H' })),
  E('na-horn-strobe-ceiling', 'Horn strobe, ceiling mount', 'notification-appliances', 'CD', triSquareIcon(naColor, { filled: false, inner: 'x', topLeft: 'CD', bottomLabel1: 'CEILING' })),
  E('na-horn-strobe-wall', 'Horn strobe, wall mount', 'notification-appliances', 'CD', triSquareIcon(naColor, { filled: false, inner: 'x', topLeft: 'CD', bottomLabel1: 'WALL' })),
  E('na-speaker-strobe-wall-wp', 'Speaker strobe, wall mount, weatherproof', 'notification-appliances', 'CD/1W', triSquareIcon(naColor, { filled: true, inner: 'x', topLeft: 'CD', topRight: 'W', bottomLabel1: 'WALL', bottomLabel2: 'WP' })),
  E('na-speaker-strobe-ceiling-wp', 'Speaker strobe, ceiling mount, weatherproof', 'notification-appliances', 'CD/1W', triSquareIcon(naColor, { filled: true, inner: 'x', topLeft: 'CD', topRight: 'W', bottomLabel1: 'CEILING', bottomLabel2: 'WP' })),
  E('na-strobe-wall-wp', 'Strobe only, wall mount, weatherproof', 'notification-appliances', 'CD', strobeSunburstIcon(naColor, { wall: true, wp: true })),
  E('na-strobe-ceiling-wp', 'Strobe only, ceiling mount, weatherproof', 'notification-appliances', 'CD', strobeSunburstIcon(naColor, { wall: false, wp: true })),
  E('na-horn-strobe-ceiling-wp', 'Horn strobe, ceiling mount, weatherproof', 'notification-appliances', 'CD', triSquareIcon(naColor, { filled: false, inner: 'x', topLeft: 'CD', bottomLabel1: 'CEILING', bottomLabel2: 'WP' })),
  E('na-horn-strobe-wall-wp', 'Horn strobe, wall mount, weatherproof', 'notification-appliances', 'CD', triSquareIcon(naColor, { filled: false, inner: 'x', topLeft: 'CD', bottomLabel1: 'WALL', bottomLabel2: 'WP' })),
  E('na-lf-sounder', 'Low frequency sounder', 'notification-appliances', 'LF', triSquareIcon(naColor, { filled: false, inner: 'F', bottomLabel1: 'LF' })),
  E('na-lf-sounder-strobe', 'Low frequency sounder strobe', 'notification-appliances', 'CD/LF', triSquareIcon(naColor, { filled: false, inner: 'F', topRight: 'CD', bottomLabel1: 'LF' })),
