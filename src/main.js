...
async function runFindAndPlace() {
  ...
  try {
    ...
    console.error('Find & Place failed:', err);
    setFindPlaceStatus('Search failed — see console.', { error: true });
  ...
  const pageNote = pagesWithNoText.length
  ...
  setFindPlaceStatus(`Placed ${placed.length} symbol${placed.length === 1 ? '' : 's'} matching "${query}"${pageNote}.`);
